...
# Claude settings
# ------------------------------------------------------------
CLAUDE_MODEL: Final[str] = "claude-sonnet-4-5"  # 利用モデル
CLAUDE_MAX_TOKENS_ENTRY_JSON: Final[int] = 1500  # EntryスコアJSONの最大出力token
...
# ------------------------------------------------------------
# Demo / Paper profile settings
# ------------------------------------------------------------
DEMO_PAPER_PROFILE_ENABLED: Final[bool] = False  # Trueでpaper専用の見た目重視プロファイルを有効化
DEMO_MAX_POSITIONS: Final[int] = 8  # DEMO時の同時保有上限
DEMO_MAX_NEW_ORDERS_PER_RUN: Final[int] = 5  # DEMO時の1runあたり新規発注上限
DEMO_FINVIZ_CANDIDATE_LIMIT: Final[int] = 60  # DEMO時のFinviz候補数
DEMO_ENTRY_ORDER_MIN_SCORE: Final[float] = 0.52  # DEMO時の実発注最小スコア
DEMO_TAKE_PROFIT_RATE: Final[float] = 1.03  # DEMO時の半利確トリガー（+3%）
DEMO_TRAILING_RATE: Final[float] = 0.985  # DEMO時の通常トレーリング幅（高値比-1.5%）
...
RECOMMENDED_POLLING_SECONDS: Final[int] = 180  # 定期起動推奨間隔（運用目安: 3分）
...
def is_demo_paper_profile_enabled() -> bool:
    return bool(USE_PAPER_ACCOUNT and DEMO_PAPER_PROFILE_ENABLED)


def get_effective_finviz_candidate_limit() -> int:
    if is_demo_paper_profile_enabled():
        return int(DEMO_FINVIZ_CANDIDATE_LIMIT)
    return int(FINVIZ_CANDIDATE_LIMIT)


def get_effective_max_positions() -> int:
    if is_demo_paper_profile_enabled():
        return int(DEMO_MAX_POSITIONS)
    return int(MAX_POSITIONS)


def get_effective_max_new_orders_per_run() -> int:
    if is_demo_paper_profile_enabled():
        return int(DEMO_MAX_NEW_ORDERS_PER_RUN)
    return int(MAX_NEW_ORDERS_PER_RUN)


def get_effective_entry_order_min_score() -> float:
    if is_demo_paper_profile_enabled():
        return float(DEMO_ENTRY_ORDER_MIN_SCORE)
    return float(ENTRY_ORDER_MIN_SCORE)


def get_effective_take_profit_rate() -> float:
    if is_demo_paper_profile_enabled():
        return float(DEMO_TAKE_PROFIT_RATE)
    return float(TAKE_PROFIT_RATE)


def get_effective_trailing_rate() -> float:
    if is_demo_paper_profile_enabled():
        return float(DEMO_TRAILING_RATE)
...
DEBUG_EXECUTION_MODE: Final[int] = 0  # 0:通常, 1:判定のみ, 2:売判定のみ, 3:買判定のみ
