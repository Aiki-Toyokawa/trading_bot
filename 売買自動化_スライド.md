...
## RasPi5 / Alpaca / Claude / Python による低コストアルゴ売買

- 寝ている間に米株を監視・売買
- AIは補助、最終判断はPythonルール
- 目標は **損小利大で勝つより負けない**
...
- RasPiで常時動く低コストな自動売買システムを作る
- 米株の夜間時間帯を使って、日本にいながら自動実行する
- 完全AI任せではなく、**AIとルールベースを分担**する
- 売買ログと統計を残し、あとから改善し続けられる形にする
...
- 個人向けAPIが扱いやすい
- `paper trading` が最初から使える
- `paper -> live` の移行が簡単
- 米株のプレ、通常、アフター帯を扱いやすい
- 将来的に仮想通貨にも広げやすい
...
- 実際の期待値は、約定履歴が十分に貯まってからでないと確定しない
- ただし設計思想は明確
- **損小利大**
- 買いの品質を厳しめにする
- 売りは `-3%` で切って、勝ちは伸ばす
...
- 電気代: `100 - 200円 / 月`
- Claude API: 稼働状況次第だが、通常日は高く、ポジション満杯の日はかなり低い
